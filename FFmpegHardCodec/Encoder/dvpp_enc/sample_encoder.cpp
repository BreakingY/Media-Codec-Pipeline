...
int32_t HmevEncoder::codec_init(int device_id) // fix
{
    hi_s32 ret;
    ...
    m_encLooper = new HmevLooper(name.c_str(), 1, 1000, device_id); // timeout 1000 ms
...
int32_t HmevEncoder::on_encode_stream()
{
    ...
    m_encParam.encCallback.encDateOutProcess(m_encParam.channelId, vencStream, m_encParam.encCallback.arg); // fix arg
...
int32_t HmevEncoder::do_creat_channel()
{
    ...
    // set the display rate only if user sets it
    if (m_encParam.userSetDisplayRate) {
        if (m_encParam.codecType == VENC_CODEC_TYPE_H264) {
            hi_venc_h264_vui h264Vui;
            hi_mpi_venc_get_h264_vui(m_encParam.channelId, &h264Vui);
            // equation in H.264: display_rate = time_scale / num_units_in_tick / 2
            h264Vui.vui_time_info.timing_info_present_flag = 1;
            h264Vui.vui_time_info.num_units_in_tick = 1;
            h264Vui.vui_time_info.time_scale = m_encParam.displayRate * 2;
            ret = hi_mpi_venc_set_h264_vui(m_encParam.channelId, &h264Vui);
            if (ret != HI_SUCCESS) {
                HMEV_HISDK_PRT(ERROR, "hi_mpi_venc_set_h264_vui [%u] failed with 0x%x!\n", m_encParam.channelId, ret);
                return ret;
            }
        } else if (m_encParam.codecType == VENC_CODEC_TYPE_H265) {
            hi_venc_h265_vui h265Vui;
            hi_mpi_venc_get_h265_vui(m_encParam.channelId, &h265Vui);
            // equation in H.265: display_rate = time_scale / num_units_in_tick
            h265Vui.vui_time_info.timing_info_present_flag = 1;
            h265Vui.vui_time_info.num_units_in_tick = 1;
            h265Vui.vui_time_info.time_scale = m_encParam.displayRate;
            ret = hi_mpi_venc_set_h265_vui(m_encParam.channelId, &h265Vui);
            if (ret != HI_SUCCESS) {
                HMEV_HISDK_PRT(ERROR, "hi_mpi_venc_set_h265_vui [%u] failed with 0x%x!\n", m_encParam.channelId, ret);
                return ret;
            }
        }
    }
