...
HmevLooper::HmevLooper(int32_t size, int32_t timeout, int device) // fix
    : m_name("looper"), m_epoll_size(size + 2), m_timeout(timeout), device_id_(device)// 2:2 more size
{
    create_epoll(m_epoll_size);
}

HmevLooper::HmevLooper(int32_t size, int device) : m_name("looper"), m_epoll_size(size + 2), m_timeout(-1), device_id_(device) // 2:2 more size // fix
{
    create_epoll(m_epoll_size);
}

HmevLooper::HmevLooper(const char* looperName, int32_t size, int32_t timeout, int device) // fix
    : m_name(looperName), m_epoll_size(size + 2), m_timeout(timeout), device_id_(device) // 2:2 more size
{
    create_epoll(m_epoll_size);
}

HmevLooper::HmevLooper(const char* looperName, int32_t size, int device) // fix
    : m_name(looperName), m_epoll_size(size + 2), m_timeout(-1), device_id_(device) // 2:2 more size
{
    create_epoll(m_epoll_size);
}
...
void HmevLooper::loop()
{
    ...
    // fix
    aclError aclRet = aclrtSetDevice(device_id_);
